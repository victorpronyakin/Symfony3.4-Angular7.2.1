...
use AppBundle\Helper\Message\MyFileMessage;
use Doctrine\ORM\EntityManager;
use pimax\Messages\Message;

/**
 * Class SendMessageFile
 * @package AppBundle\Flows\Type\SendMessage
 */
class SendMessageFile implements SendMessageInterface
{
    /**
     * @var EntityManager
     */
    ...
    public function getSendData()
    {
        ...
        if(isset($item['params']) && isset($item['params']['url']) && !empty($item['params']['url'])){
            ...
            if($this->subscriber instanceof Subscribers){
                $this->setSendItem(new MyFileMessage($this->subscriber->getSubscriberId(), $item['params']['url'], $this->quickReplies, $this->tag, $this->typePush, $this->messageType));
            }
            else{
                $this->setSendItem(new MyFileMessage($this->subscriber, $item['params']['url'], $this->quickReplies, $this->tag, $this->typePush, $this->messageType));
            }
        }
    ...
    public function getJSONData()
    {
        ...
        if(isset($item['params']) && isset($item['params']['url']) && !empty($item['params']['url'])){
            ...
            if($this->subscriber instanceof Subscribers){
                $message = new MyFileMessage($this->subscriber->getSubscriberId(), $item['params']['url'], $this->quickReplies);
            }
            else{
                $message = new MyFileMessage($this->subscriber, $item['params']['url'], $this->quickReplies);
            }
            $result = true;
        }
